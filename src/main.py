...
def portfolio():
    ...
    try:
        ...
        # Get all portfolio images (excluding About/Info images) - sorted by capture date (newest first), fallback to upload date
        images_query = Image.query.filter(Image.is_about != True).order_by(
